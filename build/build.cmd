...
powershell.exe -NoProfile -ExecutionPolicy unrestricted -Command "& { Import-Module '.\..\src\packages\psake.4.3.1.0\tools\psake.psm1'; Invoke-psake '.\Build.ps1' -task %task% -parameters @{version='%version%'} ; if ($lastexitcode) { write-host "ERROR: $lastexitcode" -fore RED; exit $lastexitcode }; if ($psake.build_success -eq $false) { exit 1 } else { exit 0 } }"
